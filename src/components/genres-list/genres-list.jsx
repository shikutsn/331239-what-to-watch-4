...
import {GENRES_ALL, GENRES_MAX_COUNT} from "../../const.js";
...
class GenresList extends PureComponent {
  constructor(props) {
    ...
    this.genresList = this._getGenresList(this.props.movies);

  }

  static _getGenresList(moviesList) {
    const genresList = removeDuplicates(moviesList.map((movie) => movie.genre));
    return [GENRES_ALL].concat(genresList).slice(0, GENRES_MAX_COUNT);
  }
  ...
  render() {
    ...
    return (
      <ul className="catalog__genres-list">
        {this.genresList.map((currentGenre) => {
